      ...
      function __DESTROY__(connection, cb) {
        ...
        async.auto({
          ...
          findRecords: function(next) {
            collection.destroyMode === 'blind' ? next() :
        ...
        function(err, results) {
          if(err) return cb(err);
          var result = results.findRecords || [];
          results.destroyRecords && results.destroyRecords[0] &&
            (result.count = results.destroyRecords[0].affectedRows || 0);
          cb(null, result);
        });
