            ...
            async.auto({
              ...
              processChildren: ['buildChildBuffers', function(next, results) {
                ...
                async.each(_query.query, function(q, next) {
                  ...
                  if (q.instructions && (qs.indexOf('SELECT * ') > -1)) {
                    var inst = (_.isArray(q.instructions) && q.instructions[0]) ? q.instructions[0] : q.instructions;
                    if (_.isArray(inst.select) && inst.select.length) {
                      qs = qs.replace('SELECT * ', 'SELECT `' + inst.child + '`.`' +
                        inst.select.join('`, `' + inst.child + '`.`') + '` ');
                    }
                  };
