...
module.exports = (function() {
              ...
              processChildren: ['buildChildBuffers', function(next, results) {
                ...
                async.each(_query.query, function(q, next) {
                  ...
                  parentRecords.forEach(function(parent) {
                    ...
                    _qs += q.qs.replace('^?^', _.isNumber(parent[pk]) ? parent[pk] : ('"' + parent[pk] + '"')) +
                      ' UNION ';

                    // @todo: do this at sequel level instead of dping string manipilation
                    instruction = _.isArray(q.instructions) && q.instructions[0] || q.instructions;
                    if ((select = _getUserSelect(instruction)) && instruction.reducedSelection) {
                      (select.indexOf(instruction.childKey) < 0) && select.push(instruction.childKey);

                      _qs = _qs.replace('SELECT *', util.format('SELECT `%s`.`%s`', instruction.child,
                        select.join('`, `' + instruction.child + '`.`')));

                      return qs += _qs;
                    }

                    instruction = _.isArray(q.instructions) && q.instructions[1];
                    if ((select = _getUserSelect(instruction)) && instruction.reducedSelection) {
                        _qs = _qs.replace(/^(\(?SELECT ).+?( AS )/, '$1^??^, `' +
                            instruction.parent + '`.`' + (q.instructions[0] && q.instructions[0].childKey || '') +
                            '`$2'); // @todo: check q.instructions[0].childKey refers to correct table
                        _qs = _qs.replace('^??^', util.format('`%s`.`%s`', instruction.child, select.join('`, `' +
                            instruction.child + '`.`')));
                    }
  ...
  function _getUserSelect (instruction) {
    return _.isArray(instruction && instruction.criteria && instruction.criteria.select) &&
      !!instruction.criteria.select.length && instruction.criteria.select;
  }
