                ...
                async.each(_query.query, function(q, next) {
                  ...
                  parentRecords.forEach(function(parent) {
                    var _qs = '',
                      ...
                      select = _.isArray(instruction && instruction.criteria.select) && instruction.criteria.select;

                    _qs += q.qs.replace('^?^', _.isNumber(parent[pk]) ? parent[pk] : ('"' + parent[pk] + '"')) + 
                      ' UNION ';

                    if (select.length && (_qs.indexOf('SELECT * ') > -1)) {
                      // precaution. @todo: check if childKey is the correct property
